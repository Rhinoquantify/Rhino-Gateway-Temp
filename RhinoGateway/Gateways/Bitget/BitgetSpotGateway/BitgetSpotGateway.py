...
websocket_url = "wss://wbs.mexc.com/raw/ws"
...
class BitgetSpotGateway(BaseGateway):
    def __init__(self, logger: RhinoLogger, rhino_collect_config: RhinoConfig, loop):
        ...
        self.rest = BitgetSpotRestGateway(self)
        self.websocket = None
...
class BitgetSpotRestGateway(RestClient):

    def __init__(self, gateway: BitgetSpotGateway):
    ...
    async def get_more_depths(self, symbol_info: SymbolInfo, callable_methods: CallableMethods = None) -> NoReturn:
        ...
        rhino_request = RhinoRequest(
            ...
            url=rest_api + "/api/v2/spot/market/orderbook",
            params=params,
            data=None,
            headers=None,
    ...
    async def on_get_more_depths(self, request: RhinoRequest, data, code: int, extra: MixInfo,
        ...
        d = data.get("data")
        rhino_depth.gateway_send_time = int(d.get("ts"))
    ...
    async def get_best_depths(self, symbol_info: SymbolInfo, callable_methods: CallableMethods = None) -> NoReturn:
        ...
        rhino_request = RhinoRequest(
            method=Method.GET.value,
            url=rest_api + "/api/v2/spot/market/tickers",
            params=params,
            data=None,
            headers=None,
    ...
    async def on_get_best_depths(self, request: RhinoRequest, data, code: int, extra: MixInfo,
                                 on_transfer: Callable = None, on_transfer_extra_data: Any = None) -> NoReturn:
        rhino_depth = get_RhinoDepth_from_MixInfo(extra)
        rhino_depth.cex_exchange = Exchange.BITGET.value
        rhino_depth.cex_exchange_sub = ExchangeSub.BITGETSPOT.value
        d = data.get("data")[0]
        rhino_depth.gateway_send_time = int(d.get("ts"))
        rhino_depth.rhino_get_time = int(time.time() * 1000)

        rhino_depth.buy_price1 = float(d.get("bidPr"))
        rhino_depth.buy_amount1 = float(d.get("bidSz"))
        rhino_depth.sell_price1 = float(d.get("askPr"))
        rhino_depth.sell_amount1 = float(d.get("askSz"))
        # self.gateway.logger.debug(
        #     f"{self.gateway.exchange_sub} depth {extra.__str__()} 获取数据消耗时间 {int(time.time() * 1000) - extra.start_time}")
        # await self.gateway.set_data(rhino_depth)
        await on_transfer(rhino_depth, on_transfer_extra_data)
    ...
    async def on_get_all_best_depths(self, request: RhinoRequest, data, code: int, extra: MixInfo,
        ...
        for d in data.get("data"):
            ...
            buy_price = d.get("bidPr")
            buy_amount = d.get("bidSz")
            sell_price = d.get("askPr")
            sell_amount = d.get("askSz")
    ...
    async def get_coin_info(self, symbol_info: SymbolInfo, callable_methods: CallableMethods = None):
        ...
        rhino_request = RhinoRequest(
            method=Method.GET.value,
            url=rest_api + "/api/v2/spot/public/coins",
            params=None,
            data=None,
            headers=None,
    ...
    async def on_get_coin_info(self, request: RhinoRequest, data, code: int, extra: MixInfo,
        ...
        for d in data.get("data"):
            ...
            withdraw_amounts = []
            transfer_feess = []
            for network in d.get("chains"):
                chains.append(network.get("chain"))
                # contracts.append(True if network.get("contract") == "1" else False)
                deposit_enables.append(True if network.get("rechargeable") == "1" else False)
                withdraw_enables.append(True if network.get("withdrawable") == "1" else False)
                withdraw_amounts.append(float(network.get("withdrawFee")))
                transfer_feess.append(float(network.get("extraWithdrawFee")))
